...
static inline void cdn_nand_wait_idle(void)
{
	uint32_t reg = 0U;
...
static inline void cdn_nand_wait_thread_ready(uint8_t thread_id)
{
	uint32_t reg = 0U;
...
static void cdn_nand_pinmux_config(void)
{
...
static int cdn_nand_last_opr_status(uint8_t thread_id)
{
	uint8_t nthreads = 0U;
	uint32_t reg = 0U;
...
/* Set feature command */
int cdn_nand_set_feature(uint8_t feat_addr, uint8_t feat_val,
							uint8_t thread_id)
{
	/* Wait for thread to be ready */
	cdn_nand_wait_thread_ready(thread_id);
	...
	return(cdn_nand_last_opr_status(thread_id));
}

/* Reset command to the selected device */
int cnd_nand_reset(uint8_t thread_id)
{
...
static void cdn_nand_set_opr_mode(uint8_t opr_mode)
{
	/* Wait for controller to be in idle state */
	cdn_nand_wait_idle();
	...
	if (CNF_OPR_WORK_MODE_SDR == opr_mode) {
	...
	} else if (CNF_OPR_WORK_MODE_NVDDR == opr_mode) {
		;
	} else {
		;
	}
...
static void cdn_nand_transfer_config(void)
{
	/* Wait for controller to be in idle state */
	cdn_nand_wait_idle();
...
static int cdn_nand_update_dev_info(void)
{
	uint32_t reg = 0U;
	...
	if (CNF_DT_UNKNOWN == dev_info.type) {
...
static int cdn_nand_host_init(void)
{
	uint32_t reg = 0U;
	int ret = 0;
	...
	/* Set operation work mode */
	cdn_nand_set_opr_mode(CNF_OPR_WORK_MODE_SDR);

	/* Set data transfer configuration parameters */
	cdn_nand_transfer_config();

	return 0;
}

int cdn_nand_init(handoff *hoff_ptr)
{
	int result = 0;

	/* NAND DFI selection based on system manager DFI register */
	result = dfi_select(hoff_ptr);
	if (result != 0U) {
		ERROR("CNF: DFI configuration failed\n");
		return result;
	}

	/* NAND pin mux configuration */
	cdn_nand_pinmux_config();

	result = cdn_nand_host_init();

	return result;
}

/* erase: Block erase command */
int cdn_nand_erase(uint32_t offset, uint32_t size)
{
	...
	cdn_nand_wait_thread_ready(CNF_DEF_TRD);
	...
	return(cdn_nand_last_opr_status(CNF_DEF_TRD));
}

/* io mtd functions */
int cdn_nand_init_mtd(unsigned long long *size, unsigned int *erase_size)
{
...
static int cdn_nand_read_page(uint32_t block, uint32_t page, uintptr_t buffer)
{
	/* Wait for thread to be ready */
	cdn_nand_wait_thread_ready(CNF_DEF_TRD);
	...
	if (cdn_nand_last_opr_status(CNF_DEF_TRD)) {
...
int cdn_nand_read(unsigned int offset, uintptr_t buffer, size_t length,
					size_t *out_length)
{
	...
	while (block <= end_block) {
		for (page = page_start; page < nb_pages; page++) {
			if ((start_offset != 0U) || (length < dev_info.page_size)) {
				/* Partial page read */
				result = cdn_nand_read_page(block, page,
