...
/* Set feature command */
int cdns_nand_set_feature(uint8_t feat_addr, uint8_t feat_val,
							uint8_t thread_id)
{
...
{
	/* ToDo: what else we can do in this function? */
...
static uint32_t cdns_nand_get_row_address(uint32_t page, uint32_t block)
{
	...
	for (uint32_t i = 0U; i < sizeof(uint32_t) * 8; i++) {
		if ((1 << i) & dev_info.npages_per_block)
			req_bits = i;
	}
...
static int cdns_nand_read_page(uint32_t block, uint32_t page, uintptr_t buffer)
{
	...
	mmio_write_32(CNF_CMDREG(CMD_REG2), (buffer & 0xFFFFFFFF));
	mmio_write_32(CNF_CMDREG(CMD_REG3), ((buffer >> 32) & 0xFFFFFFFF));
