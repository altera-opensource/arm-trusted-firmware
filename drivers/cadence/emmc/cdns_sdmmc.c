...
static void cdns_init(void);
static int cdns_send_cmd(struct mmc_cmd *cmd);
static int cdns_set_ios(unsigned int clk, unsigned int width);
static int cdns_prepare(int lba, uintptr_t buf, size_t size);
static int cdns_read(int lba, uintptr_t buf, size_t size);
static int cdns_write(int lba, uintptr_t buf, size_t size);

static const struct mmc_ops cdns_sdmmc_ops = {
	.init			= cdns_init,
...
{
	/* Reset embedded card */
	mmio_write_32((MMC_REG_BASE + SDHC_CDNS_SRS10), (7 << BVS) | (1 << BP));
	udelay(250);
	mmio_write_32((MMC_REG_BASE + SDHC_CDNS_SRS10), (7 << BVS) | (0 << BP));
	udelay(500);
...
static void cdns_init()
{
	/* Dummy function pointer for cdns_init. */
}

static int cdns_prepare(int dma_start_addr, uintptr_t dma_buff,
	size_t size)
{
	data_cmd = true;
...
static void cdns_host_set_clk(int clk)
{
	...
	mmio_write_32(MMC_REG_BASE + SDHC_CDNS_SRS11, (dtcvval << DTCV) | (sdclkfsval << SDCLKFS) |
		(1 << ICE));
	...
	mmio_write_32(MMC_REG_BASE + SDHC_CDNS_SRS11, (dtcvval << DTCV) | (sdclkfsval << SDCLKFS) | (1 << ICE) |
			(1 << SDCE));

	mmio_write_32(MMC_REG_BASE + SDHC_CDNS_SRS13, 0xFFFFFFFF);
}
...
static int cdns_set_ios(unsigned int clk, unsigned int width)
{
	...
	case MMC_BUS_WIDTH_1:
		mmio_write_32((MMC_REG_BASE + SDHC_CDNS_SRS10), BIT1 | _status);
		break;
	case MMC_BUS_WIDTH_4:
		mmio_write_32((MMC_REG_BASE + SDHC_CDNS_SRS10), BIT4 | _status);
		break;
	case MMC_BUS_WIDTH_8:
		mmio_write_32((MMC_REG_BASE + SDHC_CDNS_SRS10), BIT8 | _status);
		break;
	default:
		assert(0);
		break;
...
static int cdns_hc_set_clk(struct cdns_sdmmc_params *cdn_sdmmc_dev_mode_params)
{
	...
	mmio_write_32(MMC_REG_BASE + SDHC_CDNS_SRS11, 0);
	mmio_write_32(MMC_REG_BASE + SDHC_CDNS_SRS11, (dtcvval << DTCV) |
...
int cdns_sd_host_init(struct cdns_sdmmc_combo_phy *mmc_combo_phy_reg, struct cdns_sdmmc_sdhc *mmc_sdhc_reg)
{
	int ret = 0;
...
void cdns_sdmmc_init(struct cdns_sdmmc_params *params, struct mmc_device_info *info)
{
	assert((params != NULL) &&
	...
	mmc_init(&cdns_sdmmc_ops, params->clk_rate, params->bus_width,
		params->flags, info);
}
