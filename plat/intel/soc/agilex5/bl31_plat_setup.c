...
void bl31_plat_arch_setup(void)
{
	...
	const mmap_region_t bl_regions[] = {
		MAP_REGION2(BL31_BASE, BL31_BASE, BL31_END - BL31_BASE,
...
void bl31_plat_set_secondary_cpu_entrypoint(unsigned int cpu_id)
{
	...
	if (boot_core == 0x00) {
		...
		mmio_write_64(RSTMGR_CPUxRESETBASELOW_CPU2, (uint64_t) plat_secondary_cpus_bl31_entry >> 2);
	}
	else {
		/* Update reset vector to 0x00 */
		mmio_write_64(RSTMGR_CPUxRESETBASELOW_CPU0, (uint64_t) plat_secondary_cpus_bl31_entry >> 2);
	}
