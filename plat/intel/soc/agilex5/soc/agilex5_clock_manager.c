...
void config_clkmgr_handoff(handoff *hoff_ptr)
{
	...
	/* Set clock manager into boot mode before running configuration */
	mmio_setbits_32(CLKMGR_OFFSET + CLKMGR_CTRL,
		CLKMGR_CTRL_BOOTMODE_SET_MSK);
	/* Bypass all mainpllgrp's clocks */
	mmio_write_32(CLKMGR_MAINPLL + CLKMGR_MAINPLL_BYPASS, 0x36);
	wait_fsm();

	/* Bypass all perpllgrp's clocks */
	mmio_write_32(CLKMGR_PERPLL + CLKMGR_PERPLL_BYPASS, 0xef);
	wait_fsm();
	...
	// mmio_write_32(CLKMGR_MAINPLL + CLKMGR_MAINPLL_ENS,
			// CLKMGR_MAINPLL_ENS_RESET);
	mmio_write_32(CLKMGR_PERPLL + CLKMGR_PERPLL_EN,
			CLKMGR_PERPLL_EN_RESET);
	// mmio_write_32(CLKMGR_PERPLL + CLKMGR_PERPLL_ENS,
			// CLKMGR_PERPLL_ENS_RESET);

	/* Pass clock source frequency into scratch register */
	mmio_write_32(SOCFPGA_SYSMGR(BOOT_SCRATCH_COLD_0),
		hoff_ptr->qspi_clk_khz);
