...
typedef struct handoff_t {
	...
	uint32_t	alt_s2fuser1ctr;
	uint32_t	alt_psirefctr;
	uint32_t	qspi_clk_khz; // Temp added for clk manager. check with Siew Chin on handoff data in high level FD
	uint32_t	hps_osc_clk_hz;
	uint32_t	fpga_clk_hz;
	uint32_t	ddr_reset_type; // Temp added for clk manager. check with Siew Chin on handoff data in high level FD
	uint32_t	hps_status_coldreset; // Temp added for clk manager. check with Siew Chin on handoff data in high level FD
	//uint32_t	_pad_0x604_0x610[3]; // Temp remove due to add in extra handoff data
#endif
	/* misc configuration */
	uint32_t	misc_magic;
