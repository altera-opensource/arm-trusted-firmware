...
void deassert_peripheral_reset(void)
{
	mmio_clrbits_32(SOCFPGA_RSTMGR(PER1MODRST),
			...
			// There is no UART2 register map in MAS RTL0.5. Temp put a value
...
static int poll_idle_status_by_counter(uint32_t addr, uint32_t mask, uint32_t match, uint32_t delay_ms)
{
	int time_out = delay_ms;
...
int socfpga_bridges_enable(uint32_t mask)
{
	...
	if (brg_mask & RSTMGR_BRGMODRSTMASK_SOC2FPGA) {
		...
		if (ret < 0) {
			ERROR("S2F bridge enable: "
					"Timeout waiting for Read Reset Manager hdskack\n");
		}
	...
	if (brg_mask & RSTMGR_BRGMODRSTMASK_FPGA2SOC) {
		...
		if (ret < 0) {
			ERROR("FPGA bridge fpga handshake fpgahsreq: "
					"Timeout waiting for handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2S bridge fpga handshake f2sdram_flush_req: "
					"Timeout waiting for handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2S bridge fpga handshake f2s_flush_ack: "
					"Timeout waiting for handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2S bridge fpga handshake fpgahsack: "
					"Timeout waiting for handshake ack\n");
		}
	...
	if (brg_mask & RSTMGR_BRGMODRSTMASK_F2SDRAM0) {
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake fpgahsreq: "
					"Timeout waiting for handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake f2sdram_flush_req: "
					"Timeout waiting for handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake f2sdram_flush_ack: "
					"Timeout waiting for handshake ack\n");
		}
		...
		if (ret < 0) {
			ERROR("F2SDRAM bridge fpga handshake fpgahsack: "
					"Timeout waiting for handshake ack\n");
		}
